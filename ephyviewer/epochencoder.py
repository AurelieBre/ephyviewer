...
class EpochEncoder_ParamController(Base_ParamController):
    ...
    def create_new_label_ABC(self):
        ...
        # get next unused shortcut key
        used_keys = [p['key'] for p in self.viewer.by_label_params]
        unused_keys = [k for k in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' if k not in used_keys]
...
class EpochEncoder(ViewerBase):
    ...
    def set_layout(self):
        ...
        range_shortcut.setKey(QT.QKeySequence('*'))
        range_shortcut.activated.connect(self.range_group_toggle)
        self.range_group_box.setToolTip('Toggle with shortcut: *')
...
class EpochEncoder_ABC(ViewerBase):
    _default_params = default_params
    ...
    def set_layout(self):
        ...
        self.toolbar.addAction('New label', self.params_controller.create_new_label_ABC)
